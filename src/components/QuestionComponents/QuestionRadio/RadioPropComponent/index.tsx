...
const RadioPropComponent: FC<PropsType> = (props: PropsType) => {
  const {
    title = '问卷单选',
    isVertical = false,
    options = [],
  ...
  }, [form, isVertical, options, title, value]);

  // onValuesChange 数据从 Form ===> 画布/Redux
  const handleValueChange = (_changedValues: any, allValues: any) => {
    const { title, isVertical, options, value } = allValues;
    const newProps: PropsType = { title, isVertical, options, value, disabled: false };
    dispatch(updateSelectedComponent(newProps));

    // console.log('@allValues', allValues.options);
  };
